...
class Pins:
	# Unused pins, no need to define them across all variants
	# SPEAKER = const(25)
	# SPEAKER_EN = const(13)
	BL = 0
	BATT = const(36)
...
def init_pins(variant: int):
	if variant == 0:
		...
		pinMap[1] = const(-1)  # LED_R
		pinMap[2] = const(-1)  # LED_G
		pinMap[3] = const(-1)  # LED_B
		pinMap[4] = const(39)  # CHARGE_DETECT_PIN
		pinMap[5] = const(34)  # SD_DETECT_PIN
